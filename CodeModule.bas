...
'-- unzip.dll Userfunctions Structure
...
'-- This Assumes unzip.dll Is In Your \Windows\System Directory!
Private Declare Function Wiz_SingleEntryUnzip Lib "unzip.dll" _
...
Private Declare Sub UzpVersion2 Lib "unzip.dll" (uzpv As UZPVER)
...
'ZPOPT is used to set options in the unzip.dll
Private Type ZPOPT
    fSuffix As Long
    fEncrypt As Long
    fSystem As Long
...
'This assumes unzip.dll is on your path.
Private Declare Function ZpInit Lib "unzip.dll" _
(ByRef Zipfun As ZIPUSERFUNCTIONS) As Long ' Set Zip Callbacks

Private Declare Function ZpSetOptions Lib "unzip.dll" _
(ByRef Opts As ZPOPT) As Long ' Set Zip options

Private Declare Function ZpGetOptions Lib "unzip.dll" _
() As ZPOPT ' used to check encryption flag only

Private Declare Function ZpArchive Lib "unzip.dll" _
...
'-- Callback For unzip.dll - Receive Message Function
...
'-- Callback For unzip.dll - Print Message Function
Public Function UZDLLPrnt(ByRef fname As UNZIPCBChar, ByVal x As Long) As Long
  ...
  '-- Gets The unzip.dll Message For Displaying.
  For xx = 0 To x - 1
    If fname.ch(xx) = 0 Then Exit For
    s0 = s0 & Chr(fname.ch(xx))
  Next
...
'-- Callback For unzip.dll - DLL Service Function
Public Function UZDLLServ(ByRef mname As UNZIPCBChar, ByVal x As Long) As Long
    ...
    s0 = ""
    '-- Get unzip.dll Message For processing
    For xx = 0 To x - 1
...
'-- Callback For unzip.dll - Password Function
...
'-- Callback For unzip.dll - Report Function To Overwrite Files.
