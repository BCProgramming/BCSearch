...
Public Function LZW_CompressData(Source() As Byte) As Byte()
    Dim dst() As Byte
    Dim r As Long
    ReDim dst(0 To UBound(Source) * 1.2 + 36000)
    r = qlz_compress(Source(0), dst(0), UBound(Source) + 1)
    ReDim Preserve dst(0 To r - 1)
    LZW_CompressData = dst
End Function

Public Function LZW_GetSize(Source() As Byte) As Long
    LZW_GetSize = qlz_size_decompressed(Source(0))
End Function

Public Function LZW_DecompressData(Source() As Byte) As Byte()


    Dim dst() As Byte
    Dim r As Long
    Dim size As Long
    size = LZW_GetSize(Source)
