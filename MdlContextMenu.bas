...
End Function
' Returns the low 16-bit integer from a 32-bit long integer
Private Function HiByte(wValue As Integer) As Byte
...
Function FolderFromItem(hWndOwner As Long, vItem As Variant, _
   ...
   Dim retval As Long, foldpointer As Long
    ...
    If varType(vItem) = vbString Then
        ...
        folder.ParseDisplayName hWndOwner, 0, StrPtr(CStr(vItem) & vbNullChar), _
                                cParsed, pidlItem, 0
    Else
...
' returning to the code today (12/22/2009 22:48:09) I have to flippin clue what I was planning with cPopupMenu; I imagine I was going to try to do
'something to get the hMenu we create for the context menu that is created here to be handled by the popupmenu, or to get the popupmenu's hwnd and use that.
